 ...
 * 404 is included: on static hosts (GitHub Pages / Netlify) there is no backend,
 * so /api/* resolves to the host's 404 or SPA-fallback page.
 */
export function isApiUnreachable(err) {
  const s = err?.status;
  return !s || s >= 500 || s === 404;
}
...
async function request(method, path, body) {
  ...
  // Static-host SPA fallback returns 200 + HTML for unknown paths — that is
  // not the API answering, it's the absence of one. Surface as unreachable.
  const contentType = res.headers.get('content-type') || '';
  if (res.ok && !contentType.includes('application/json')) {
    throw Object.assign(
      new Error('API unreachable — non-JSON response (static host fallback).'),
      { status: 0 },
    );
  }
